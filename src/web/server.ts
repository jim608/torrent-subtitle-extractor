...
import { SubtitleProcessor, type ProcessorOptions, type SubtitleCandidate } from '../core/processor';
  ...
  app.post('/api/extract', upload.single('torrent'), async (req, res) => {
    ...
    try {
      if (!req.file && !req.body.magnetLink) {
        return res.status(400).json({ error: 'No file uploaded or magnet link provided' });
      }
      ...
      const rateLimit = req.body.rateLimit || '512k';
      ...
      // Parse torrent
      console.log(`[Web] Parsing torrent: ${torrentSource}`);
      ...
      for (const candidate of candidates) {
        try {
          ...
          results.push(`✗ ${path.basename(candidate.path)} (${String(error).slice(0, 50)})`);
        }
      }
      ...
      // Optional: Keep temp dir for download, or delete immediately
